...
ABS_THRESHOLDS_S3S2 = [-400.0, -300.0, -200.0, -150.0, -100.0, -50.0, -25.0]
PERC_THRESHOLDS_S3S2 = [-0.4, -0.3, -0.25, -0.2, -0.15, -0.10, -0.05]

ABS_THRESHOLDS_S2S1 = [10.0, 25.0, 50.0, 75.0, 100.0, 150.0, 200.0]
PERC_THRESHOLDS_S2S1 = [0.2, 0.7, 1.4, 2.0, 3.0, 5.0, 7.0]

def kepler_config(data_id, palette, opacity_val):
    return {
        "version": "v1",
        "config": {
            "mapState": {
...
if st.session_state.mode == "S3_S2":
    st.markdown(
        "<h3>Difference in commuting-related CO₂ emissions at the selected percentage of the remote-working population VS at the remote-working population percentage in S3</h3>",
        unsafe_allow_html=True
    )
    ...
    gdf["Absolute change in emissions"] = (gdf["absolute_change"] * (1 - factor)).round(1)
    ...
    gdf["color_abs_hex"] = gdf["Absolute change in emissions"].apply(
    ...
    df_abs = gdf[["Absolute change in emissions", "geometry_json"]].copy()
    ...
    cfg_abs = kepler_config("absolute_change", COLOR_PALETTE, opacity_val)
    cfg_perc = kepler_config("percentage_change", COLOR_PALETTE, opacity_val)
    ...
    with col1:
        ...
        make_color_legend(
            "Legend: Absolute change in emissions (tonnes)",
            COLOR_PALETTE[::-1],
    ...
    with col2:
        ...
        make_color_legend(
            "Legend: Percentage change in emissions (%)",
            COLOR_PALETTE[::-1],
...
# S2 vs S1
    ...
    st.markdown(
        "<h3>Difference in commuting-related CO₂ emissions at the selected percentage of the remote-working population VS at the remote-working population percentage in S2</h3>",
        unsafe_allow_html=True
    )
