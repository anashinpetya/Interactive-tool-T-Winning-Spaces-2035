...
def kepler_config_lines(data_id, palette):
    return {
        "version": "v1",
        "config": {
            "mapState": {"latitude": 60.26, "longitude": 25, "zoom": 8.7},
            "mapStyle": {
                        "id": "carto_dark",
                        "label": "Carto Dark",
...
if st.session_state.mode == "S3_S2":
    ...
    with col1:
        ...
        map_abs = KeplerGl(height=500, data={"absolute_change": df_abs}, config=cfg_abs)
    ...
    with col2:
        ...
        map_perc = KeplerGl(height=500, data={"percentage_change": df_perc}, config=cfg_perc)
